"""
Create user-defined mask; for example: beam stop, ASIC edges,
known bad regions, etc.
...
from createMaskBase import CreateMasks
...
class CreateUserMasks(CreateMasks):
    ...
    def __init__(self, base_dir, module, temperature, outfile_name, manual,
                 asic_edge):
    ...
    def run(self):
        ...
        dset_manual_mask = self.masks.create_dataset("manual",
                                                     shape=(352, 128, 512),
                                                     dtype=bool)
        if self.manual is not None:
        ...
        print("Manual % of cells masked: ",
              100 * manual_mask.flatten().sum() / manual_mask.size)

        # ASIC edges
        asic_edge_mask = np.zeros((352, 128, 512), dtype=bool)
        dset_asic_edge_mask = self.masks.create_dataset("asic_edges",
                                                        shape=(352, 128, 512),
                                                        dtype=bool)
        if self.asic_edge:
        ...
        print("ASIC edges masked, %: ",
              100 * asic_edge_mask.flatten().sum() / asic_edge_mask.size)
...
if __name__ == "__main__":
    ...
    obj = CreateUserMasks(base_dir, module, temperature, outfile_name, manual,
                          asic_edge)
