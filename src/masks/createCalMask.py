...
class CreateCalMasks(CreateMasks):
    ...
    def __init__(self, base_dir, module, temperature, outfile_name, current, tint, element, 
        ...
        self.gain_input_path = os.path.join(self.input_dir, "drscs", self.current)
        ...
        self.dark_filepath = os.path.join(self.input_dir, 'dark', self.dark_filename)
        
        # X-ray
        self.xray_filename = "photonSpacing_{}_xray_{}.h5".format(self.module, self.element)
        self.xray_filepath = os.path.join(self.input_dir, 'xray', self.xray_filename)
    ...
    def run(self):
        ...
        dset_dark_offset_mask = self.masks.create_dataset("dark_offset", shape=self.dark_offset.shape, dtype=bool)
        dark_offset_mask = create_cal_mask(self.dark_offset, self.dark_offset_range)
        ...
        # Noise (dark width)
        dset_noise_mask = self.masks.create_dataset("noise", shape=self.noise.shape, dtype=bool)
        ...
        gains_hi_mask = create_cal_mask(self.gains[0, ...], self.gains_range[0])
        gains_med_mask = create_cal_mask(self.gains[1, ...], self.gains_range[1])
        gains_low_mask = create_cal_mask(self.gains[2, ...], self.gains_range[2])
        ...
        #dset_photon_spacing_mask = self.masks.create_dataset("photon_spacing", shape=self.photon_spacing.shape, dtype=bool)
        dset_photon_spacing_mask = self.masks.create_dataset("photon_spacing", shape=(128, 512), dtype=bool)
        photon_spacing_mask = create_cal_mask(self.photon_spacing, self.photon_spacing_range)
    ...
    def get_gain_data(self):
        ...
        self.gains = np.concatenate((gains_top, gains_bot), axis=1) # shape: 3 x 128 x 512 x 352
        self.gain_offsets = np.concatenate((gain_offsets_top, gain_offsets_bot), axis=1) # shape: 3 x 128 x 512 x 352
        self.error_code = np.concatenate((error_code_top, error_code_bot), axis=0) # shape: 128 x 512 x 352
        self.warning_code = np.concatenate((warning_code_top, warning_code_bot), axis=0) # shape: 128 x 512 x 352
        
        # transpose to be in same order as dark and xray - for now at least
        self.gains = self.gains.transpose((0, 3, 1, 2)) # shape: 3 x 352 x 128 x 512
        self.gain_offsets = self.gain_offsets.transpose((0, 3, 1, 2)) # shape: 3 x 352 x 128 x 512
        self.error_code = self.error_code.transpose((2, 0, 1)) # 352 x 128 x 512
        self.warning_code = self.warning_code.transpose((2, 0, 1)) # 352 x 128 x 512
    ...
    def create_row(self, asic_list, dset_list):
        ...
        for asic in asic_list:
            ...
            if asic == asic_list[0]:
                ...
                gain_offsets_row = np.concatenate((gain_offsets_row, gain_offsets_tmp), axis=2)
                error_code_row = np.concatenate((error_code_row, error_code_tmp), axis=1)
                warning_code_row = np.concatenate((warning_code_row, warning_code_tmp), axis=1)
