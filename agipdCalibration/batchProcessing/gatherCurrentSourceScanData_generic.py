...
class GatherData():
    def __init__(self, rel_file_path, file_base_name, output_file_name):
        ...
        self.column_specs = [[15, 1], [26, 2], [37, 3], [48, 4]]
    ...
    def run(self):
        ...
        # e.g.totaldataCount, 128, 512 -> dataCountPerFile, 352, 2, 128, 512

        self.images_per_burst = 352
        ...
        dataCountPerFile = int(f[self.dataPathInFile].shape[0] / 2 / 352)
        dataCount = dataCountPerFile * self.number_of_files
        ...
        dset_analog = saveFile.create_dataset("analog",
                                              shape=(dataCount, 352, 128, 512),
                                              chunks=(dataCount, 352, 64, 64),
        ...
        try:
            for i in np.arange(len(self.files)):
                for j in np.arange(self.number_of_files):
                    ...
                    rawData = f[self.dataPathInFile][..., 0:128, 0:512]
                    print('took time:  ' + str(time.time() - t))
                    ...
                    dc_start = j * dataCountPerFile
                    dc_stop = (j + 1) * dataCountPerFile
                    l = np.arange(3 - i, 512, 4)
                    print('Reshaping analog data')
                    tmp = rawData[:, :, 0, :, :]
                    analog[dc_start:dc_stop, :, 0:64, np.arange(3 - i, 512, 4)] = tmp[..., 0:64, np.arange(3 - i, 512, 4)]
                    analog[dc_start:dc_stop, :, 64:, np.arange(i, 512, 4)] = tmp[..., 64:, np.arange(i, 512, 4)]

                    print('Reshaping digital data')
                    tmp = rawData[:, :, 1, :, :]
                    digital[dc_start:dc_stop, :, 0:64, np.arange(3 - i, 512, 4)] = tmp[..., 0:64, np.arange(3 - i, 512, 4)]
                    digital[dc_start:dc_stop, :, 64:, np.arange(i, 512, 4)] = tmp[..., 64:, np.arange(i, 512, 4)]
                    print('took time:  ' + str(time.time() - t))
...
if __name__ == "__main__":
    ...
    output_file_name = "/gpfs/cfel/fsds/labs/processed/kuhnm/currentSource_chunked.h5"

    GatherData(rel_file_path, file_base_name, output_file_name)
